...
- **Worktrees need `uv sync --frozen`.** `[tool.uv.sources]` declares
  ...
  Side effect: after merging a worktree branch to main and running
  `uv sync` on main, the editable-install paths in `uv.lock` will flip
  from `../../../deriva-{mcp-core,ml}` (worktree-relative) back to
  `../deriva-{mcp-core,ml}` (main-relative). Commit this — main's
  vantage point is the canonical one, and CI will sync from main.
