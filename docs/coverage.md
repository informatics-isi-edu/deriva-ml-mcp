...
| deriva_ml_list_assets | tools/asset.py | False | v1.2 (issue #2) | Paginated list of rows in one asset table. Cursor pattern shared with `list_datasets` / `list_workflows`: `limit` / `after_rid` / `preflight_count`, capped at 1000. (`deriva_ml_list_asset_tables` was retired in v3.4 -- asset table discovery is now resource-only via `ml/assets/{schema}`.) |
...
| deriva_ml_reindex_rows | tools/maintenance.py | False | v1.4 (issue #9) | Cross-user freshness bridge for the per-user-trio (Dataset / Workflow / Execution) RAG sources. v1.3's surgical re-index handles the calling user's OWN mutations; this tool refreshes after-the-fact mutations by OTHER users (or by non-MCP clients like Chaise). `target=None` (default) refreshes all of the calling user's per-user sources for the catalog; `target="<table>:<rid>"` refreshes one source surgically. `mutates=False` -- cache refresh, not catalog state change. Returns `{"reindexed": {"dataset": N, "workflow": M, "execution": K}}` with success counts. Per-RID failures are swallowed and logged; partial success is normal. Mirrors the v1.1 `deriva_ml_reindex_vocabularies` precedent for "framework can't trigger automatically; ship a manual bridge." Automatic cross-user fan-out (Options 1-3 in #9) deferred -- load profile is deployment-specific and no demand signal yet. |
| deriva_ml_create_vocabulary | tools/vocabulary.py | True | (e2e Phase 6 follow-up) | ML-aware create-vocabulary that delegates to `DerivaML.create_vocabulary`. Picks up the deriva-ml project name as the curie prefix (so terms get `{project}:{RID}` identifiers) and the domain schema as the default placement; refreshes the navbar by default. Prefer over deriva-mcp-core's generic `create_vocabulary` on any catalog with the deriva-ml schema installed. Both produce physically-identical tables (deriva-py's `VocabularyTableDef` underlies both); the difference is in defaults and side effects. `add_term` (from deriva-mcp-core) populates terms after creation for either. |
...
| deriva://catalog/workflow-types | resources.py | merged-then-restructured | deriva://catalog/{h}/{c}/ml/vocabularies/{schema}/Workflow_Type | resources/ml.py | Originally folded into the `registries` one-shot snapshot. v3.4 retired `registries` and replaced it with the schema-scoped `ml/vocabularies/{schema}` / `ml/vocabularies/{schema}/{vocab_name}` hierarchy that surfaces all vocabularies (built-in + user-defined). |
...
| deriva://catalog/dataset-types | resources.py | merged-then-restructured | deriva://catalog/{h}/{c}/ml/vocabularies/{schema}/Dataset_Type | resources/ml.py | Originally folded into the `registries` one-shot snapshot. v3.4 retired `registries`; see the `ml/vocabularies/{schema}` hierarchy. |
...
| deriva://vocabulary/{vocab_name} | resources.py | restored-as-resource | deriva://catalog/{h}/{c}/ml/vocabularies/{schema}/{vocab_name} | resources/ml.py | v3.4 brought a per-vocab resource back to the ML plugin under a schema-scoped URI. Returns full term contents (six fields including CURIE/URI). Core's `lookup_term` / `list_vocabulary_terms` tools remain the paginated counterpart. |
...
| deriva://catalog/asset-tables | resources.py | restructured | deriva://catalog/{h}/{c}/ml/assets/{schema} | resources/ml.py | Landed in v1.2 as `ml/asset-tables` (flat). v3.4 replaced it with the schema-scoped `ml/assets/{schema}`, retiring the `deriva_ml_list_asset_tables` tool alongside. |
| deriva://catalog/assets | resources.py | landed | deriva://catalog/{h}/{c}/ml/assets/{schema}/{asset_table} | resources/ml.py | Per-asset-table snapshot (capped at `_MAX_LIMIT`). Per-row shape is `AssetSummary` -- shared with `deriva_ml_list_assets` so the resource and the paginated tool cannot drift. |
